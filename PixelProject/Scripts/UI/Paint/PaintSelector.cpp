...
PaintSelector::PaintSelector()
{
   ...
   const auto handleKeyFunction = std::bind(&PaintSelector::HandleMouseEvent, this, std::placeholders::_1, std::placeholders::_2);
...
void PaintSelector::GenerateTextures(const GameSettings* game_settings)
{
   ...
   if (shaderManager->CreateShaderProgramFromFiles(GetShaderMask(ShaderMask::MVertex, ShaderMask::MFragment), SHADER_NAME, "shaders/orthoUI") == nullptr)
   {
   ...
   _projection_transform = glm::ortho(0.0f, static_cast<float>(game_settings->screen_size.x), static_cast<float>(game_settings->screen_size.y), 0.0f, -1.0f, 1.0f);
}
