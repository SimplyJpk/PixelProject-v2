   ...
   : _game_settings(game_settings)
{
   ...
   for (auto& [chunkIndex, chunk] : _chunks)
   {
      ...
      {
         IVec2 neighbourIndex = chunkIndex + Chunk::GetDirVector(i);
   ...
   InputManager::GetInstance()->AddMouseListener(MouseCode::MouseMiddle, "MousePenDraw", [this](const SDL_Event& event, bool is_down) {
      if (is_down)
      {
         IVec2 pos = IVec2(event.motion.x, event.motion.y);
   ...
   for (int iteration = 0; iteration < 4; iteration++) {
      for (int yChunkIndex = iteration % 2; yChunkIndex < WORLD_SIZE.y; yChunkIndex += 2) {
         for (int xChunkIndex = (iteration / 2) % 2; xChunkIndex < WORLD_SIZE.x; xChunkIndex += 2) {
            UpdateChunk({xChunkIndex, yChunkIndex});
         }
      }
...
void WorldSimulator::UpdateChunk(const IVec2& chunk_index)
{
   ...
   asio::post(_thread_pool, [this, chunk_index]() {
      ...
      // Pixel Update
      for (auto xIndex = X_UPDATE_ORDER[_current_update_order][0]; xIndex != X_UPDATE_ORDER[_current_update_order][1]; xIndex += X_UPDATE_ORDER[_current_update_order][2])
      {
         ...
         for (auto yIndex =  Y_UPDATE_ORDER[_current_update_order][0]; yIndex != Y_UPDATE_ORDER[_current_update_order][1]; yIndex += Y_UPDATE_ORDER[_current_update_order][2])
         {
            ...
            for (const auto& updateDir : pixelTypeUpdateDirections)
            {
               ...
               uint16_t neighbourPixelIndex = localPixelIndex + Pixel::INDEX_DIRECTION_CHANGE[updateDir];
   ...
   for (int xVal = 0; xVal < World::SIZE_X; xVal++)
   {
      for (int yVal = 0; yVal < World::SIZE_Y; yVal++)
      {
         ...
         glm::vec3 modelPosition = glm::vec3(
            (((xVal)+1) * Chunk::SIZE_X * extraScale) - (Chunk::HALF_X * extraScale) + xVal, // + xVal is for fake grid TODO make a proper visual grid
...
void WorldSimulator::Pen(const IVec2& position, int size, const bool override_pixels)
{
   ...
   penTopLeft = {std::max(std::min(penTopLeft.x, World::SIZE_X * Chunk::ARRAY_MAX_X), 0),
                    std::max(std::min(penTopLeft.y, World::SIZE_Y * Chunk::ARRAY_MAX_Y), 0)};
   
   penBottomRight = {std::max(std::min(penBottomRight.x, World::SIZE_X * Chunk::ARRAY_MAX_X), 0),
