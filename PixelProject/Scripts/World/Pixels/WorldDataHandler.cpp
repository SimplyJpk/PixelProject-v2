   ...
   for (auto& pixel : _pixel_types)
   {
      ...
      for (auto i = 0; i < pixel.colour_count; ++i)
      {
         if (_pixel_colour_map.contains(pixel.type_colours[i]))
            DEBUG_WARNING_LOG("[WorldDataHandler] Duplicate pixel colour {} found for pixel type {}.", pixel.type_colours[i], static_cast<uint32_t>(pixel.pixel_type));
...
void WorldDataHandler::SetUniformData(Shader* shader)
{
   ...
   for (int i = 0; i < TYPE_COUNT; i++)
   {
      ...
      const int colourCount = static_cast<int>(pixel.colour_count);
      ...
      for (short colourIndex = 0; colourIndex < pixel.colour_count; colourIndex++)
      {
         ...
         {
            const auto colourLoc = shader->GetUniformLocation(std::format("u_Pixels[{}].colours[{}]", pixel.pixel_index, static_cast<int>(colourIndex)));
