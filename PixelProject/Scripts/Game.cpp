...
void Game::OnDrawGUI(float delta_time)
{
   ...
   // FIXME: What
   ImGui::Text("Duration in ms of each sand update, over roughly %d updates or (%.2f seconds)", maxHistory, maxHistory * (1 / _fixed_step_step_time));
   ImGui::Text("Sand Updates occur in fixed time steps of %.2f ms, roughly %.2f updates per second", _fixed_step_step_time, 1000 / _fixed_step_step_time);
   ...
   if (ImPlot::BeginPlot("Sand Update Duration"))
   {
      ...
      ImPlot::SetupAxes("Sand Updates","Time in Millisecond",ImPlotAxisFlags_Lock,ImPlotAxisFlags_Lock|ImPlotAxisFlags_RangeFit|ImPlotAxisFlags_Opposite);
