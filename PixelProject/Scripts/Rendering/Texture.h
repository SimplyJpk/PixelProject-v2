...
public:
   ...
   void UpdateTextureData(const T& data)
   {
      ...
      auto internalFormat = FORMAT_DATA_TYPES[static_cast<int>(_format)][static_cast<int>(TextureFormatData::InternalFormat)];
   ...
   GLuint GetHandle() const INLINE_RETURN(_texture_id)
   int GetHeight() const INLINE_RETURN(_height)
   int GetWidth() const INLINE_RETURN(_width)
   TextureFormat GetFormat() const INLINE_RETURN(_format)
