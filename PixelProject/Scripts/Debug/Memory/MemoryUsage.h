...
public:
   ...
   int GetCurrentMemory() const { return _current_frame_memory; }
   int GetReferenceMemory() const { return _reference_frame_memory; }
   
   int GetMemory(const bool use_accurate = false);
   const std::vector<float>* GetMemoryHistory() const { return &_memory_history; }
